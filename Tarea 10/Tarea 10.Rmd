...
Finalmente, se tiene que la Precisión Negativa es de  0.901235 lo que sigue siendo
alto pero es más pequeña que la indicada en el otro modelo.

En general, los tres índices de calidad analizados indicaron una buena calidad
del modelo para predecir a los pasajeros que sobrevivieron o no. 

Comparando el primer modelo con este, el modelo 1 es mejor si se quiere predecir
si no sobrevivieron y el modelo 2 es mejor prediciendo a los sobrevivientes.
Si se basa en la cantidad de índices que presentan mejor resultado, en ese caso
...
Como se puede observar, el nivel de error es relativamente bajo y la precisión es bastante acertada. Según se indicó en el inciso, el error debe ser sumamente bajo para evitar errores en el envío de los paquetes. Asumiendo que se contaba con este modelo desde el principio, contemplando que existe un total de 9298 observaciones y  teniendo en cuenta el error aproximado de 0.072, entonces se tiene que al rededor de 665 números serías clasificados de manera errónea lo que implicaría que, como máximo, cerca 665 paquetes paquetes serían enviados a la dirección incorrecta, lo que para una empresa postal sería fatal pues representa un número alto de clientes insatisfechos. Por lo tanto, dados los resultados de este modelo, se preferiría la observación humana sobre el algoritmo.
...
Como se evidencia, los resultados son los mismos, por lo tanto, no es necesario el uso de un número mayor $k$ para los cálculos. En este caso, la conclusión en la misma que la anterior.
...
train_data_nums <- train_data %>% filter(Numero %in% nums) %>% droplevels()

# Se filtra el set de datos para pruebas.
test_data_nums <- test_data %>% filter(Numero %in% nums) %>% droplevels()
```

Finalmente se procede a repetir lo realizado en los incisos anteriores.
```{r}
# Se genera el modelo.
model_nums <- train.knn(Numero~., data = train_data_nums, kmax = 21)

# Se obtienen las predicciones.
prediction_nums   <- predict(model_nums, test_data_nums, type = "class")
...
```
Tal y como se muestra, para los números 3, 5 y 8, el nivel de precisión aumenta. Nuevamente bajo la misma lógica del inciso anterior, si se contara con este modelo desde el inicio y contemplando la tasa de error de aproximadamente 0.055, de los números predichos, cerca de 123 serían erróneos, lo cual representa, como máximo, 123 paquetes enviados incorrectamente, que aunque es un máximo, resulta inaceptable para una compañía postal.
