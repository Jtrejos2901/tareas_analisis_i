...
for (i in 2:ncol(titanic_datos_cat_pred)) {
 ...
 colSums(is.na(titanic_datos_num_pred))
 colSums(is.na(titanic_datos_cat_pred))
```
...
modelo_titanic
```
El resultado del modelo indica que el número óptimo de vecinos es 11.
...
```{r}
MC_titanic <- confusion.matrix(ttesting_titanic, prediccion_titanic)
general.indexes(mc = MC_titanic)
```
Se puede observar que el modelo predice bien que 156 pasajeros del Titanic no 
sobrevivieron pero falla en la predicción de 37 casos. Además, predice 
correctamente que 61 pasajeros sobrevivieron al incidente pero falló en la predicción
de 6. En general, el número de aciertos del modelo para ambas categorías es mayor que la 
cantidad de fallos, lo cual, parece indicar que el modelo es bueno.

Se deben analizar otras medidas para determinar qué tan buenos son los resultados del modelo.
La función muestra algunos índices de calidad,interpretando se tiene que la 
Precisión Global (Overaal Accuracy), es decir, el porcentaje de predicciones correctas con respecto al total de predicciones,  es de 0.83 lo que es bastante bueno.

Además, la Precisión Positiva, que corresponde al porcentaje de sobrevivientes predichos correctamente, es de 0.622449 lo cual, también es bueno.

Finalmente, se tiene que la Precisión Negativa, que corresponde al porcentaje de no sobrevivientes predichos correctamente, es de 0.962963. Entonces, la predicción de no sobrevivientes
es mejor que la de sobrevivientes.
