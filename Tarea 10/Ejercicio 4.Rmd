...
```{r}
titanic_datos_num <- cbind(titanic_datos$Survived, titanic_datos_num)
colnames(titanic_datos_num)[1] <- "Survived"
lista_poder_pred <- list()
for (i in 2:ncol(titanic_datos_num)) {
  variable  <- names(titanic_datos_num)[i]
  lista_poder_pred[[i-1]] <- numerical.predictive.power(titanic_datos_num, "Survived", variable)
}
# Caso de prueba
#variable  <- names(titanic_datos)[3]
#numerical.predictive.power(titanic_datos, "Survived", "SibSp")
```
